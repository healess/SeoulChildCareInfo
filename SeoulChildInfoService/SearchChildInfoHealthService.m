//
//  SearchChildInfoHealthService.m
...
@implementation SearchChildInfoHealthService
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	SearchChildInfoHealthServiceContent *searchChildInfoHealthServiceContent= [[SearchChildInfoHealthServiceContent alloc] init];
	[searchChildInfoHealthServiceContent passParaToXML:curID];
	
	NSLog(@"테이블 클릭 실행"); 
	
	CGRect newFrame = searchChildInfoHealthServiceContent.view.frame;
	newFrame.size.height = newFrame.size.height -1;
	searchChildInfoHealthServiceContent.view.frame = newFrame;
	subview=searchChildInfoHealthServiceContent.view;	
	[self.view addSubview:subview];
	 */
	SearchChildInfoHealthServiceContent *searchChildInfoHealthServiceContent =
	[[SearchChildInfoHealthServiceContent alloc] initWithNibName:@"SearchChildInfoHealthServiceContent" bundle:nil];
	[searchChildInfoHealthServiceContent passParaToXML:curID];
	[self.navigationController  pushViewController:searchChildInfoHealthServiceContent animated:YES];
	[searchChildInfoHealthServiceContent release];
}
