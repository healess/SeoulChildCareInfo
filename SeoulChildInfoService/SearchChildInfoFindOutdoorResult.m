//
//  SearchChildInfoFindOutdoorResult.m
...
@implementation SearchChildInfoFindOutdoorResult
...
- (void)goNextPage {
	inputPageNum++;
	[self passParaToXML:inputPageNum sido:inputRegionType outdoorType:inputoutdoorType];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	SearchChildInfoFindOutdoorDetail *searchChildInfoFindOutdoorDetail =
	[[SearchChildInfoFindOutdoorDetail alloc] initWithNibName:@"SearchChildInfoFindOutdoorDetail" bundle:nil];
	[searchChildInfoFindOutdoorDetail passParaToXML:curID];
	[self.navigationController  pushViewController:searchChildInfoFindOutdoorDetail animated:YES];
	[searchChildInfoFindOutdoorDetail release];
...
//입력 XML실행 부분
//-(void)passParaToXML
-(void)passParaToXML:(int *)inputPageNum sido:(NSString*)regionValue outdoorType:(NSString*)outdoorType
{
	
	
	
	childInfoServiceList = [[NSMutableArray alloc] init];
	inputoutdoorType = outdoorType;
	inputRegionType = regionValue;
	NSString *soapMessage = [NSString stringWithFormat:
							 ...
							 "<searchKeyword/>\n"
							 "<outdoorSido>%@</outdoorSido>\n"
							 "<outdoorGugun></outdoorGugun>\n"
							 "<outdoorType>%@</outdoorType>\n"
							 "</q0:ChildcareInfoListRequest>\n"
							 "</soapenv:Body>\n"
							 "</soapenv:Envelope>\n",[[NSString alloc] initWithFormat:@"%i", inputPageNum],inputRegionType,inputoutdoorType  //파라미터 수정으로 테스트 
							 ];
